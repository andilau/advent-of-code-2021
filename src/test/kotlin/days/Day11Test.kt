...
class Day11Test {
    ...
    inner class Part1 {
        @Test
        fun `Small example generation should transition to generation from step 0 to 1`() {
            assertThat(Day11(smallExampleGenerations[0]).step()).isEqualTo(smallExampleGenerations[1])
        }

        @Test
        fun `Small example generation should transition to Generation from step 1 to 2`() {
            assertThat(Day11(smallExampleGenerations[1]).step()).isEqualTo(smallExampleGenerations[2])
        }

        @TestFactory
        fun `Generation X should transition to Generate Y`(): List<DynamicTest> =
            largeExampleGenerations.keys.windowed(2)
                .map { (step, nextstep) ->
        ...
        @Test
        fun `After step 10, there have been a total of 204 flashes`() {
            assertThat(Day11(largeExampleGenerations.getValue(0)).flashCount(10)).isEqualTo(204)
        }
    ...
    inner class Part2 {
        @Test
        fun test() {
            assertThat(Day11(largeExampleGenerations.getValue(0)).partTwo()).isEqualTo(195)
        }
    }

    val smallExampleGenerations = listOf(
        """
        11111
        19991
        19191
    ...
    val largeExampleGenerations = listOf(
        0 to """
            5483143223
            2745854711
            5264556173
